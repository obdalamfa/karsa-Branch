# scene_base.py — kelas Scene dan pembangun ruangan indoor.
#
# Berkas ini hasil penggabungan dua sisi yang KEDUANYA membawa hal yang tidak
# bisa dibuang:
#   - Kelas `Scene` dari cabang visual membawa `paint=` (sistem Zone di
#     game/scenes/zone_paint.py). town.py meng-import `Zone, BATU_ALUN` dan
#     mati tanpa itu.
#   - `_build_indoor_room()` dan `_add_indoor_atmosphere()` dari feature/3d-mobs
#     membangun interior yang jauh lebih kaya, dan enam scene (klinik, warung,
#     bengkel, studio, rumah kaca, gunung) sudah memanggilnya.
#
# Keduanya bisa hidup bersama karena `_build_indoor_room` sisi 3d-mobs adalah
# SUPERSET: empat argumen pertamanya identik dengan versi lama, sisanya
# opsional (w, h, door_x, door_y, extra_builder, theme). Jadi semua pemanggil
# lama tetap sah tanpa diubah.
