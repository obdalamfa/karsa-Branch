...
class EntitiesManager:
    ...
    def load_scene(self, scene_name: str):
        ...
        for i, w in enumerate(s.wild_entities):
            ...
            px, py = w['x'] * TS, w['y'] * TS
            # Wild entity — ukuran & warna sesuai jenis
            vis_scale, vis_col = _WILD_VISUALS.get(w['kind'], (0.28, color.rgb(88, 95, 78)))
            e = Entity(model='quad', position=(px, GH + 0.25, py),
                       scale=vis_scale, billboard=True, color=vis_col)
            self.wild_ents[i] = e
    ...
    def _spawn_mobs_for_scene(self):
        ...
        for i, mob in enumerate(s.mobs):
            ...
            actor.scale = sc
            if moff:
                actor.y = moff   # angkat model (mis. kelelawar) — sync_visuals tak ubah y

            # Semua mob dungeon = artefak relik (batu/terakota/perunggu)
            _apply_relic_material(actor, model_name)

            # HP Bar — warna suram, gaya Disco Elysium (bukan merah terang)
